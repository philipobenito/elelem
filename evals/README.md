...
The same applies to run outputs, and more urgently, because a run creates them without being asked. `skill-creator` defaults to writing results into `<skill-name>-workspace/` as a sibling of the skill directory, which here would resolve to `skills/design-grill-workspace/` and put a full transcript archive into the install payload. Always point runs at `evals/workspace/` instead. That path and any `*-workspace/` directory are gitignored.
...
python -m scripts.run_loop \
  --eval-set <repo>/evals/design-grill-trigger.json \
  --skill-path <repo>/skills/design-grill \
  --model <model-id> \
  --max-iterations 5 \
  --verbose
```
...
|-----------------------------------------|----------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|
| `design-grill-trigger.json`             | `design-grill`             | `debugging`, `work-on-ticket`, `design-committee`, below-threshold trivial edits, read-only questions                                                   |
| `design-committee-trigger.json`         | `design-committee`         | `design-grill`, `debugging`, `work-on-ticket`, hands-off phrasing on trivial work, "committee" as feature vocabulary                                    |
| `design-review-trigger.json`            | `design-review`            | `requesting-code-review`, visual and UI design critique, ADR review, `design-grill` itself                                                              |

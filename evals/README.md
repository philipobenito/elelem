...
|------------------------------------|-----------------------|------------------------------------------------------------------------------------------------------------------------------------|
| `debug-investigation-trigger.json` | `debug-investigation` | `design-grill-me`, `work-review-receive`, `work-tdd` mid-cycle, `work-verification`, ticket pickup, mechanical dependency bumps    |
| `design-grill-me-trigger.json`     | `design-grill-me`     | `debug-investigation`, `design-recovery`, `design-committee`, below-threshold trivial edits, read-only questions                   |
| `design-committee-trigger.json`    | `design-committee`    | `design-grill-me`, `debug-investigation`, `design-recovery`, hands-off phrasing on trivial work, "committee" as feature vocabulary |
| `design-review-trigger.json`       | `design-review`       | `work-review-request`, visual and UI design critique, ADR review, `design-grill-me` itself                                         |
| `design-handoff-trigger.json`      | `design-handoff`      | `design-recovery`, `work-implementation`, tracker housekeeping, "ticketing system" as feature vocabulary                           |
| `design-recovery-trigger.json`     | `design-recovery`     | `design-handoff`, `work-implementation`, `debug-investigation` via bug tickets, triage questions, design-less tickets              |
| `work-implementation-trigger.json` | `work-implementation` | `design-grill-me`, `design-recovery`, `design-handoff`, `debug-investigation`, "fast path" as code vocabulary                      |
...
`work-implementation` fires only with an approved design in hand, so its positives all assert approval and its negatives are the surrounding lifecycle: no design yet, design on a ticket, design being ticketed, and the defect path. It inherits the retired fast-path set's vocabulary collisions (a code-level hot-path optimisation, and a security review of one) and its behavioural case: "this is dead simple, skip the ceremony" with an approved design is a positive, because the sizing step, not the user's assertion, earns that verdict.

...
6. **Invoke `design-review`.** Use `Skill` against the consolidated summary. Surface substantive change notes to the user before continuing. If `design-review` returns a non-Approved, non-escalation result (e.g. change requests), incorporate the required changes into the design summary, re-present the updated sections to the user for approval, and invoke `design-review` again before proceeding. If `design-review` escalates, stop and ask the user how to proceed.

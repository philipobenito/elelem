...
- You **MUST** be in plan mode. The router enters it before handing off. If you arrived here without it, call `EnterPlanMode`; it is a deferred tool in some sessions, so load it with `ToolSearch` (`select:EnterPlanMode`) first if a direct call fails. If plan mode genuinely cannot be entered in this environment, tell the user and continue the design step anyway. A design captured without plan mode is weaker than one captured with it, and far better than no design at all.
...
3. **Contextualised approaches.** Propose 2-3, each anchored to existing code. "Option A follows the pattern already in `src/services/`, the lowest friction. Option B introduces a new pattern; the trade-off is inconsistency until everything migrates." Lead with the recommendation and explain why it is the recommendation.
...
4. **Contextualised approaches**: A) follow the existing middleware pattern, emitting events on a Go channel that a new `audit` package consumes. B) introduce a third-party event bus. C) skip middleware entirely and instrument each admin handler. Recommend A, because it is the only one that matches a pattern already in the tree.

...
**Codeless tasks.** Mark `codeless: true` on a task whose files carry no production code: there is nothing for TDD to bind, and the task's proof is its `verifyCommand`. The marking is earned only where the project itself establishes that the content has no test framework, its project instructions defining a proving command rather than a test suite, never by convenience; mixed work splits into code tasks and codeless tasks. The marking is a prediction the reviewer re-checks against the real diff, and it is inert on a SIMPLE run, which carries its own carve-out.

**Tasks with no test framework.** Mark `noTestFramework: true` on a task whose files DO carry production code, in a project that has no test framework for those files: there is again nothing for TDD to bind, and the proof is again the `verifyCommand`. Terraform and other HCL, shell scripts, SQL migrations, Dockerfiles and CI pipeline logic are production code that most repositories carry no framework for. The earning bar is the one `codeless` sets, the project's own instructions defining a proving command rather than a test suite, and "writing the tests would be hard" never meets it.

The two markings are mutually exclusive, and pre-flight throws on a task carrying both: `codeless` says there is no production code, `noTestFramework` says there is production code and no framework for it. Do not read the rest across from `codeless`. Production code is the expected output here, so `behaviour_introduced` is not constrained to `'none'` and the implementer describes what it introduced as on any ordinary task. The marking lifts the test-first requirement and nothing else: the task is reviewed and verified exactly as every other task is. The reviewer re-checks it against the real diff and returns a `FRAMEWORK:` finding if tests appear or a framework capable of hosting them turns up, and the marking is inert on a SIMPLE run.

Present every `noTestFramework` marking with its evidence before launching, the way the sizing table is presented: stated, not gated. `../../rules/common/testing.md` permits skipping TDD for exactly this case, and the evidence is what makes the claim checkable by anyone reading afterwards.
...
For each drained task present: what was implemented and what the reviewer found, including deferred Minor findings; the verification evidence with commands, exit codes and counts; the RED to GREEN evidence, the behaviour statement on a SIMPLE run, the verification evidence alone for a codeless task, or for a `noTestFramework` task the verification evidence together with what the implementer reported introducing, because production code is present there and the verification evidence is its only proof; and a `git diff` scoped to the task's `files`, because an unscoped diff would show other tasks' work as though it were this one's. For an isolated task, first bring its owned files from the worktree named in its result into the main tree, then present and commit as normal, and remove the worktree once its task has committed.
...
- **`codelessFalsified` is true.** A task marked codeless turned out to need production code or tests. The same partial-edit situation applies, so the same three options go to the user, then re-enter at Decomposition with the work split into code tasks and codeless tasks.
- **`frameworkFalsified` is true.** A task marked `noTestFramework` turned out to have a test framework available for its files, so the test-first requirement applied after all. The same partial-edit situation applies, so the same three options go to the user, then re-enter at Decomposition with the marking dropped so TDD binds across the task, splitting off separately only those files the discovered framework does not cover.
...
| Marking a task `noTestFramework` because writing tests is hard | The marking asserts the project has no framework for those files. Convenience is not evidence, and the reviewer checks.  |

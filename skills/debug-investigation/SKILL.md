...
A first-instance investigation is lead-driven and single-context: you read the evidence and you test the hypotheses, and nothing in this section changes that default. Read-only `Agent` dispatch is permitted in exactly two windows among the phases below: concurrent checks of genuinely independent hypotheses within one Phase 5 cycle, and Phase 2 reproduction variations that run without state-changing environment setup; investigative dispatch is forbidden in Phases 1, 3, 4, 6 and 7. Inline `Workflow` scripts are excluded outright, because a script recreates the hands-off launch-to-return window the steering model above rejects. A dispatched check's return **MUST** carry the captured output or code excerpt supporting its result; a result arriving without it leaves the hypothesis untested, neither confirmed nor eliminated. Announce a dispatch before launch inside the surfacing the skill already requires, never as a blocking question: the Phase 4 ranked list names which hypotheses are claimed independent and on what evidence, and that they will be checked concurrently in the coming cycle, and the phase-boundary surfacing entering Phase 2 names which reproduction variations will run in parallel. Once the Refocus Rule or the Investigation Budget has triggered in the current investigation, any dispatch requires explicit user direction: you never initiate dispatch at that point, while the user can direct anything, including parallel checks. A dispatched agent never re-ranks or reassesses, never holds a user conversation, never makes the refocus or budget-trigger direction decision, never grants the Phase 6 approval, and never writes; the Phase 6 conversation is yours, held against evidence you have yourself seen.
...
**The Refocus Rule.** If you have read five files without forming a clear hypothesis, counting in aggregate files read by you and by any agent you dispatched, stop gathering evidence; a dispatched return that cannot enumerate the files it read counts as reaching the five-file threshold. A hypothesis is clear when you can state its What, its Evidence and its Predicted observation distinctly, as Phase 4 requires; clarity is whether those three can be stated, not how confident you are in them. Stop and present what you know, what you have ruled out, and what remains uncertain, and ask the user which direction to pursue. Unfocused investigation wastes time and context; the instinct to "just read one more file" is how investigation spirals begin, and the rule exists to cut the spiral early.

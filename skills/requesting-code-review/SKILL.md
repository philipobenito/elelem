...
Dispatch a `code-reviewer` subagent on `claude-haiku-4.5`, or if it is unavailable `gemini-3.5-flash`, or if that is unavailable `gpt-5.4-mini`, with the filled template:

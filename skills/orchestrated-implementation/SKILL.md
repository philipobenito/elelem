...
Each task carries, in `description` as prose: scene-setting for where it fits in the design, the files it touches and what each is for, and its acceptance criteria. The recommended model sits alongside them or in `metadata`. Pick one concrete model at the tier whose task signal matches the task, resolved per `../_shared/subagent-dispatch.md`: a clearly specified task touching few files sits at the Low-cost default, and multi-file integration starts at Standard escalation. Escalate beyond the signal only with justification.
